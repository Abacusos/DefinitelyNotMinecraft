...
class Handle {
    public:
    constexpr static IndexType invalidValue = std::numeric_limits<IndexType>::max();
    IndexType                  index        = IndexType(0);

    operator bool() const { return index != invalidValue; }

    friend bool operator==(const Handle& lhs, const Handle& rhs) { return lhs.index == rhs.index; }
