...
namespace dnm
{
Input::Input(Camera* camera, GLFWwindow* window, BlockWorld* world, Config* config, GizmoData* gizmoRendering) :
    m_camera {camera}, m_window {window}, m_world {world}, m_config {config}, m_gizmoData {gizmoRendering} {
...
void Input::processInput(GLFWwindow* window, TimeSpan dt) {
    ...
    if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS) {
        m_config->insertionMode = static_cast<u32>(BlockWorld::BlockAction::Destroy);
    }
    ...
    if (positionBlock) {
        v3 position {
          positionBlock->chunkIndex.x * BlockWorld::chunkLocalSize + positionBlock->positionWithinChunk.x,
          positionBlock->positionWithinChunk.y,
          positionBlock->chunkIndex.y * BlockWorld::chunkLocalSize + positionBlock->positionWithinChunk.z};
