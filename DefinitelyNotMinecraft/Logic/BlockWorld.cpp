...
BlockWorld::getChunkData(glm::ivec2 chunkPosition) const {
  std::lock_guard g{m_chunkDataMutex};
  const auto it = m_chunkData.find(chunkPosition);
  if (it == m_chunkData.end()) {
    assert(false);
...
std::optional<BlockWorld::BlockPosition>
BlockWorld::getFirstTracedBlock(Camera *camera) {
...
std::optional<BlockWorld::BlockPosition>
BlockWorld::getBlockPosition(v3 position) {
  std::lock_guard g{m_chunkDataMutex};
...
BlockWorld::getPositionWithOffset(const BlockPosition &position, i32 x, i32 y,
                                  i32 z) {
