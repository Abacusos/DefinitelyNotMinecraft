...
namespace dnm
{
    ...
    void BlockRenderingModule::recompileShadersIfNecessary(bool force)
    {
        ...
        std::array defines = {
            ShaderManager::Define{
                m_interner->addOrGetString(localWGSizeX),
                std::to_string(m_blockWorld->chunkLocalSize)
            },
            ShaderManager::Define{
                m_interner->addOrGetString(localWGSizeY),
                std::to_string(1)
            },
            ShaderManager::Define{
                m_interner->addOrGetString(localWGSizeZ),
                std::to_string(m_blockWorld->chunkLocalSize)
            }
        };
    ...
    bool BlockRenderingModule::updateBlockWorldData(v3 cameraPosition)
    {
        ...
        const glm::ivec2 cameraChunk{
            cameraPosition.x / BlockWorld::chunkLocalSize,
            cameraPosition.z / BlockWorld::chunkLocalSize
        };

        const glm::ivec2 min{
            cameraChunk.x - m_config->loadCountChunks,
