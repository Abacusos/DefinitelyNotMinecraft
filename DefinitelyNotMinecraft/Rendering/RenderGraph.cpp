...
namespace dnm {
RenderGraph::RenderGraph(Renderer *renderer, Camera *camera)
...
void RenderGraph::build() {
  ...
  for (const auto &node : m_nodes) {
    if (node->shouldExecute()) {
      ...
      m_sortedNodes.emplace_back(std::move(semaphore), std::move(commandBuffer),
                                 node.get());
    }
  }
}
...
void RenderGraph::execute() {
  for (auto i = 0u; i < m_sortedNodes.size(); ++i) {
    ...
    auto executionResult =
        node.node->execute(executionData, node.commandBuffer);
    ...
    const vk::SubmitInfo computeInfo{
        array,
        executionResult.flags == vk::PipelineStageFlagBits::eNone
            ? vk::ArrayProxyNoTemporaries<const vk::PipelineStageFlags>{}
            : executionResult.flags,
        *node.commandBuffer, *node.semaphore};
    executionResult.queue.submit(computeInfo);
  }

  if (!m_sortedNodes.empty()) {
    m_renderer->finishDrawFrame(
        m_sortedNodes[m_sortedNodes.size() - 1].semaphore, m_frameBufferIndex);
  }
}
} // namespace dnm
