...
GizmoRenderingNode::GizmoRenderingNode(Config *config, Renderer *renderer,
                                       ShaderManager *shaderManager,
                                       BlockWorld *blockWorld,
                                       StringInterner *interner,
                                       GizmoData *gizmoData)
    : m_config{config}, m_renderer{renderer}, m_shaderManager{shaderManager},
      m_blockWorld{blockWorld}, m_interner{interner}, m_gizmoData{gizmoData} {
  ...
  m_vertexBuffer =
      BufferData(physicalDevice, device,
                 GizmoData::reservedGizmoSpace * sizeof(VertexGizmo),
...
GizmoRenderingNode::execute(const ExecutionData &executionData,
...

  {
    copyToDevice(
        ...
        std::span<const VertexGizmo>(m_gizmoData->m_verticesGizmo.data(),
                                     m_gizmoData->m_occupiedVertexPlaces));
    m_gizmoData->reset();
