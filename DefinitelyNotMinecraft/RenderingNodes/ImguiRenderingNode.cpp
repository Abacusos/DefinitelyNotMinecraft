...
namespace dnm
{
ImguiRenderingNode::ImguiRenderingNode(Config* config, Renderer* renderer) : m_config {config}, m_renderer {renderer} {
...
ImguiRenderingNode::~ImguiRenderingNode() {
...
IRenderingNode::ExecutionResult ImguiRenderingNode::execute(const ExecutionData& executionData, vk::raii::CommandBuffer& commandBuffer) {
    ZoneScoped;
    ...
    const vk::RenderPassBeginInfo renderPassBeginInfo(*m_renderPass, *frameBuffer, vk::Rect2D(vk::Offset2D(0, 0), extent), clearValues);
    ...
    return {{vk::PipelineStageFlagBits::eColorAttachmentOutput}, m_renderer->getGraphicsQueue()};
}

void ImguiRenderingNode::recreatePipeline() {
    m_renderer->waitIdle();
}
...
void ImguiRenderingNode::uploadFontTexture() {
    ...
    registerDebugMarker(device, m_fontTexture.imageData.image, "Imgui Font Texture");

    m_renderer->oneTimeSubmit([&](const vk::raii::CommandBuffer& commandBuffer) { m_fontTexture.setImage(commandBuffer, pixels); });
