...
namespace
{
    constexpr std::array<std::string_view, 3u> searchTokens = {"local_size_x", "local_size_y", "local_size_z"};
}

ShaderReflector::ShaderReflector(StringInterner* interner) : m_interner {interner} {}
    ...
    for (u32 i = 0u, size = tokenized.size(); i < size; ++i) {
        ...
        for (auto searchToken : searchTokens) {
            if (token == searchToken && i + 2 < size) {
                ...
                if (tokenized [i + 2].find_first_not_of("0123456789") == std::string_view::npos) {
